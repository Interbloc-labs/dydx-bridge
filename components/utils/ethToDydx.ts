...
export const DYDX = bech32Chain("DYDX", "dydx");

export const ethToDydx = (ethAddress: string) => {
  const data = ETH.decoder(ethAddress);
  return DYDX.encoder(data);
};

export const dydxToEth = (dydxAddress: string) => {
  const data = DYDX.decoder(dydxAddress);
  return ETH.encoder(data);
};
